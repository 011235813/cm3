"""
Implementation of the CM3 policy gradient with credit-function.
"""
...
class Alg(object):
    ...
    def __init__(self, experiment, dimensions, stage=1, n_agents=1,
        ...
        """
        Inputs:
        experiment: string, options are 'sumo', 'particle'
        dimensions: dictionary containing tensor dimensions
                     (h,w,c) for tensor
                     l for 1D vector
        stage: integer curriculum stage of CM3
        ...
        self.l_goal_others = (n_agents-1) * self.l_goal
    ...
    def create_networks(self, stage):
        ...
        with tf.variable_scope("Q_global_main"):
            if self.experiment == 'particle':
                self.Q_global = networks.Q_global_1output(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_other_agents, self.action_others, stage=stage)
            elif self.experiment == 'sumo':
                self.Q_global = networks.Q_global_sumo(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_other_agents, self.action_others, self.v_goal_others, stage=stage)
        with tf.variable_scope("Q_global_target"):
            if self.experiment == 'particle':
                self.Q_global_target = networks.Q_global_1output(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_other_agents, self.action_others, stage=stage)
            elif self.experiment == 'sumo':
                self.Q_global_target = networks.Q_global_sumo(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_other_agents, self.action_others, self.v_goal_others, stage=stage)

        # Q_n(s,a^m)
        if self.n_agents > 1 and self.use_Q_credit:
            with tf.variable_scope("Q_credit_main"):
                if self.experiment == 'particle':
                    self.Q_credit = networks.Q_credit(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_m, self.v_state_other_agents, stage=stage)
                elif self.experiment == 'sumo':
                    self.Q_credit = networks.Q_credit_sumo(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_m, self.v_state_other_agents, self.v_goal_others, stage=stage)
            with tf.variable_scope("Q_credit_target"):
                if self.experiment == 'particle':
                    self.Q_credit_target = networks.Q_credit(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_m, self.v_state_other_agents, stage=stage)
                elif self.experiment == 'sumo':
                    self.Q_credit_target = networks.Q_credit_sumo(self.v_state_one_agent, self.v_goal, self.action_one, self.v_state_m, self.v_state_other_agents, self.v_goal_others, stage=stage)

        # V(s,g^n), used as ablation at stage 2
        if self.n_agents > 1 and self.use_V:
            with tf.variable_scope("V_main"):
                self.V = networks.V_particle_ablation(self.v_state_one_agent, self.v_goal, self.v_state_other_agents)
            with tf.variable_scope("V_target"):
                self.V_target = networks.V_particle_ablation(self.v_state_one_agent, self.v_goal, self.v_state_other_agents)
    ...
    def get_assign_global_to_credit_ops(self):
        """Returns ops that assign value of Q_global network to Q_credit network.
    ...
    def run_actor_target(self, local_others, local_v, goals, epsilon, sess):
        """Gets actions from the slowly-updating target policy.
    ...
    def create_Q_credit_train_op(self):
        """Defines op for training the credit function."""
    ...
    def create_Q_global_train_op(self):
        """Defines op for training global Q function."""
    ...
    def create_V_train_op(self):
        """Defines op for training global value function."""
    ...
    def create_policy_gradient_op(self):
        """Defines the policy gradient, for either Stage 1 or Stage 2."""
    ...
    def train_step(self, sess, batch, epsilon, idx_train,
        ...
        if self.experiment == 'sumo':
            feed[self.v_goal_others] = goals_others
        ...
        if self.n_agents > 1 and self.use_Q_credit:
            ...
            if self.experiment == 'sumo':
                goals_others_rep = np.reshape(np.repeat(np.reshape(goals_others, [-1, self.n_agents*self.l_goal_others]), self.n_agents, axis=0), [-1, self.l_goal_others])
            ...
            feed = {self.v_state_one_agent : s_n_next_rep,
            ...
            if self.experiment == 'sumo':
                feed[self.v_goal_others] = goals_others_rep
        ...
        if self.n_agents == 1:
            ...
            feed = {self.v_state_one_agent : s_rep,
                    self.v_goal : goals_self_rep,
                    self.action_one : actions_cf,
                    self.v_state_other_agents : v_global_others, # not used
                    self.action_others : actions_others_1hot}
            if self.experiment == 'sumo':
                feed[self.v_goal_others] = goals_others # not used
            ...
            if self.use_Q_credit: 
                ...
                if self.experiment == 'sumo':
                    goals_others_rep = np.repeat(goals_others_rep, self.l_action, axis=0)
                ...
                feed = {self.v_state_one_agent : s_n_rep,
                        self.v_goal : goals_self_rep,
                        self.action_one : actions_cf,
                        self.v_state_m : s_m_rep,
                        self.v_state_other_agents : s_others_rep}
