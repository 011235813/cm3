...
def train_function(config):
    ...
    for idx_episode in range(1, N_train+1):
        ...
        if (idx_episode >= pretrain_episodes) and (idx_episode % episodes_per_train == 0):
            for idx_epoch in range(epochs):
                # Sample batch of transitions from replay buffer
                batch = buf.sample_batch(batch_size)
                if summarize and idx_episode % period == 0 and idx_epoch == 0:
        ...
        if idx_episode % period == 0:
            ...
            if experiment == 'sumo':
                ...
                if stage==1 and np.sum(r_local_eval) > 9.0:
                    saver.save(sess, '../saved/%s/model_good_%d.ckpt' % (dir_name, idx_episode))
            s = '%d,%d,%.2f,' % (step, idx_episode, reward_global_century/float(period))
